...
test_data_engine = DataEngine(data_dir=args.data_dir, data_split=args.test_split, with_intent=args.with_intent)
...
if args.dual_inference_weight < 1.0 or True:
    dual_inf = DualInference(nlu_weight=args.dual_inference_weight,
                            nlg_weight=args.dual_inference_weight,
                            lm=lm,
                            made=made,
                            lm_weight=args.lm_weight,
                            made_weight=args.made_weight,
                            data_engine=train_data_engine)
    model.set_dual_inference(dual_inf)
else:
    model.set_dual_inference(None)
if args.train:
    try:
        model.train(
                epochs=args.epochs,
    ...
    model.test(
        ...
        test_nlg=False,
        epoch=-1,
        sample_size=20 if args.dataset != 'e2enlg' else 17
    )
