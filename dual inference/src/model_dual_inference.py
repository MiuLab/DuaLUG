...
class DSL:
    ...
    def set_dual_inference(self, dual_inference):
        self.dual_inference = dual_inference

    def test(self, batch_size,
             criterion_nlu, criterion_nlg,
             test_nlu=True, test_nlg=True,
             sample_size=20, epoch=-1):
    ...
    def run_test_nlu_batch(self, batch, criterion, scorer=None,
                      testing=False, optimize=True, max_norm=None, beam_size=20,
                      teacher_forcing_ratio=0.5, retain_graph=False, result_path="./nlu_outputs.txt", sample_size=1,
                      supervised=True, reinforce=False):
        self.nlu.eval()
        ...
        if self.with_intent:
            intent_prediction = best_intent.cpu() #torch.argmax(best_intent.cpu(), dim=1)
            intent_prediction = intent_prediction.clone().numpy()
        ...
        return torch.tensor([0]), slot_logits, outputs_indices, intent_logits, intent_prediction, None, 0, 0
    ...
    def run_test_nlg_batch(self, batch, criterion, scorer=None,
        ...
        if self.dual_inference is not None:
            best = self.dual_inference.forward_nlg(attrs, logits, decisions, nlu_slot_seqs, beam_size, self.nlu)
        else:
            best = decisions[:, 0].detach()
    ...
    def _record_nlu_test_result(self,
        ...
        encoder_input = [
            ...
            [x for x in self.data_engine.untokenize(sent) if x != '[PAD]']
            for idx, sent in enumerate(encoder_input)
        ]
        #decoder_label = [
            # self.data_engine.tokenizer.untokenize(sent, sf_data[idx])
            #[self.data_engine.nlu_index2slot(x) for x in sent if x != -1]
        ...
        if self.nlu_output_file is None:
            self.nlu_output_file = open(result_path, 'w')
            self.nlu_output_index = 0
            self.nlu_output_file.write("index\tinput\tpredict\ttarget\n")
        for idx in range(len(encoder_input)):
            self.nlu_output_file.write("{}\t{}\t{}\t{}\n".format(self.nlu_output_index+idx,
            ' '.join(encoder_input[idx]),
            ' '.join(prediction[idx][:len(encoder_input[idx])-1]),
            ' '.join(decoder_label[idx])))
        self.nlu_output_index += len(encoder_input)
