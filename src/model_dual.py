...
class Dual:
    ...
    def train(self, epochs, batch_size, criterion_nlu, criterion_nlg,
        ...
        def train_nlg():
            ...
            for b_idx, batch in enumerate(pbar):
                self.nlg_batches += 1
                batch_loss, batch_logits, batch_decode_result, _, _ = self.run_nlg_batch(
                        batch,
                        criterion_nlg,
                        scorer=scorer,
                        testing=False,
        ...
        def train_joint():
            ...
            for batch_nlg, batch_nlu in pbar:
                batch_loss_nlg, batch_loss_nlu = train_nlg_nlu_joint_batch(
                    batch_nlg,
        ...
        with torch.no_grad():
            ...
            with open(os.path.join(self.log_dir, 'validation', 'test_dump_SNS.txt'), 'w') as fw:
                ...
                for b_idx, batch in enumerate(tqdm(self.test_nlg_data_loader)):
                    batch_amount += 1
                    encoder_input, decoder_label, refs, sf_data = batch
                    _, _, decode_result, _, _ = self.run_nlg_batch(
                        batch,
                        criterion_nlg,
                        teacher_forcing_ratio=0.0,
                        testing=True,
    ...
    def run_nlg_batch(self, batch, criterion, scorer=None,
        ...
        # print(sup_loss, rl_loss)
        return sup_loss + rl_loss, logits, outputs, nlg_joint_prob, reward
