...
class LM:
    ...
    def run_batch(self, batch, criterion, testing=False, result_path=None):
        ...
        _BOS = self.train_data_engine.tokenizer.token_to_id('[CLS]')
        _EOS = self.train_data_engine.tokenizer.token_to_id('[SEP]')
        prefix = torch.full((batch['target'].size(0), 1), _BOS, dtype=torch.long, device=batch['target'].device)
        suffix = torch.full((batch['target'].size(0), 1), _EOS, dtype=torch.long, device=batch['target'].device)
        inputs = torch.cat((prefix, batch['target']), dim=-1).cuda()
        pads = (batch['target'] == 0).long()
        targets = (batch['target'] - pads) # ignored index = -1
        targets = torch.cat((targets, suffix), dim=-1).cuda()
        # pdb.set_trace()
